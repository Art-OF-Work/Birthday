...
x = setInterval(function() {
  ...
  let w = (c.width = window.innerWidth),
      ...
      calc = {
          totalWidth: opts.charSpacing * Math.max(...opts.strings.map(str => str.length)) // Corrected calculation
      },
      Tau = Math.PI * 2,
      TauQuarter = Tau / 4,
      letters = [];
  ...
  Letter.prototype.reset = function (delay) {
    this.phase = 'firework';
    this.tick = -delay; // Start tick in the negative based on delay
    this.spawned = false;
